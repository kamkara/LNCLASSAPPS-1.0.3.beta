...
class LevelsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create level" do
    assert_difference('Level.count') do
      post levels_url, params: { level: { slug: @level.slug, title: @level.title, user_id: @level.user_id } }
    end
  ...
  test "should update level" do
    patch level_url(@level), params: { level: { slug: @level.slug, title: @level.title, user_id: @level.user_id } }
